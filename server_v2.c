...
/***待改进问题***/
//1. 创建线程可以提早（在连接成功时就pthread_create），以免其中一个用户还在登录时，其他用户无法登录（即阻塞在read用户昵称处）
//2. 私聊功能
//（已解决）3. 进场welcome
//4. 文件传输
...
#define MAXCAPACITY 2	//聊天室最大用户数量
...
int main()
{
	...
	while(1)
	{
		...
		if (usernum >= MAXCAPACITY){
			...
			//用户信息配置
			for (i=0; i<MAXCAPACITY && users[i].id!=0; i++);//查找空闲的用户信息存储位置
			//提示用户设定昵称，并存储
			...
			users[i].id = userid++;//从1开始编号users.id,id=0表示空
			printf("LOGIN:\tUser:%s\tIP:%s\tID:%d\n", users[i].name, users[i].ip, users[i].id);
...
void *service_thread(void *useri)
{
	//struct userinfo current_user = *(struct userinfo *)user;
	int i = *(int *)useri;
       	int n;
